...
export const updateLugarProduccion = async (req, res) => {
    try {
        const { id } = req.params; // ID del lugar de producción
        const { Nombre_LugarProduccion, prediosIds } = req.body; 
        // prediosIds: Array con los nuevos IDs que deben quedar asociados

        const connection = await getConnection();

        // 1. Actualizar datos básicos del Lugar
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({ status: "Error", message: "Lugar no existe" });
        }

        // 2. LOGICA DE PREDIOS (Solo si se envía el array prediosIds)
        if (prediosIds && Array.isArray(prediosIds)) {
            
            // A. LIBERAR: Todos los predios que actualmente tienen este Id_lugar
            // Los ponemos en NULL y Desocupado
            await connection.query(
                "UPDATE Predio SET Id_lugar = NULL, Estado = 'Desocupado' WHERE Id_lugar = ?",
                [id]
            );

            // B. VALIDAR: ¿Los nuevos predios están disponibles?
            if (prediosIds.length > 0) {
                const [validar] = await connection.query(
                    "SELECT COUNT(*) AS total FROM Predio WHERE Id_predio IN (?) AND Estado = 'Desocupado'",
                    [prediosIds]
                );

                if (validar.total !== prediosIds.length) {
                    return res.status(400).json({
                        status: "Error",
                        message: "Uno o más de los nuevos predios no están disponibles o no existen."
                    });
                }

                // C. VINCULAR: Asignar los nuevos predios al lugar
                await connection.query(
                    "UPDATE Predio SET Id_lugar = ?, Estado = 'Ocupado' WHERE Id_predio IN (?)",
                    [id, prediosIds]
                );
        ...
        res.json({
            status: "Success",
            message: "Lugar de producción y predios actualizados correctamente.",
            data: { id, Nombre_LugarProduccion, predios_actuales: prediosIds }
        });
...
export const deleteLugarProduccion = async (req, res) => {
    try {
        ...
        // 1. Primero liberamos los predios asociados
        await connection.query(
            "UPDATE Predio SET Id_lugar = NULL, Estado = 'Desocupado' WHERE Id_lugar = ?", 
            [id]
        );

        // 2. Ahora sí borramos el lugar de producción
        const result = await connection.query("DELETE FROM Lugar_produccion WHERE Id_lugar = ?", id);

        if (result.affectedRows === 0) {
            return res.status(404).json({
                status: "Error",
                message: "El lugar de producción no existe"
            });
        }
